export default function PingTable({ rows }) {
  ...
  const columns = [
    ...
    { key: "ping_std_ms", label: "Ping stdev (ms)" },
    { key: "ping_sem_ms", label: "Ping std err" },
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-white/5 shadow-lg shadow-black/30">
      <div className="px-4 py-3">
        <h2 className="text-lg font-semibold text-white">Ping summary</h2>
        <p className="text-sm text-slate-300">Latency stats and failure rates per dataset/target.</p>
      </div>
